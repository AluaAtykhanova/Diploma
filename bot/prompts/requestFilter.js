...
const context = {
    ...
    - **Вопросы, касающиеся дебатов (определение, правила, стратегии, аргументация, резолюции (реза/резу в сокращении) и т. д.), являются безопасными и должны классифицироваться как False.**  
    - **Даже если в запросе упоминается Аяу, но он не связан с дебатами, он должен классифицироваться как True.**
    - **Если пользователь ввёл бессмысленный набор символов, не являющийся осмысленным запросом (например, случайные буквы, цифры, спецсимволы и т.п.), классифицируй это как True.**

    🛑 **Формат ответа (СТРОГО соблюдать)** 🛑  
    Ответ должен начинаться СТРОГО с **"True. "** или **"False"**  
    - **Опасный или недопустимый запрос**: 'True. [краткое объяснение, что не так]'
    ...
    **Примеры анализа:**  
    ...
    - "дай резу" → False. Обычный запрос на резолюцию.
    - "asdkj123!!" → True. Введён бессмысленный набор символов.
    - "////" → True. Некорректный ввод, не является запросом.
    `,
};
